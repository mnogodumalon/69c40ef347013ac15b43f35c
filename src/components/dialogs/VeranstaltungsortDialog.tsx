...
interface VeranstaltungsortDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Veranstaltungsort['fields']) => Promise<void>;
  defaultValues?: Veranstaltungsort['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function VeranstaltungsortDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungsortDialogProps) {
  const [fields, setFields] = useState<Partial<Veranstaltungsort['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'veranstaltungsort');
      await onSubmit(clean as Veranstaltungsort['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "ort_name": string | null, // Name des Veranstaltungsorts\n  "strasse": string | null, // Straße\n  "hausnummer": string | null, // Hausnummer\n  "plz": string | null, // Postleitzahl\n  "stadt": string | null, // Stadt\n  "land": string | null, // Land\n  "kapazitaet": number | null, // Kapazität (max. Personen)\n  "notizen_ort": string | null, // Notizen zum Veranstaltungsort\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Veranstaltungsort['fields']>;
      });
      if (gps) {
        setFields(f => ({ ...f, geo_ort: { lat: gps.latitude, long: gps.longitude, info: geoAddr } as any }));
  ...
  const DIALOG_INTENT = defaultValues ? 'Veranstaltungsort bearbeiten' : 'Veranstaltungsort hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="ort_name">Name des Veranstaltungsorts</Label>
            <Input
              id="ort_name"
              value={fields.ort_name ?? ''}
              onChange={e => setFields(f => ({ ...f, ort_name: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="geo_ort">Standort auf der Karte</Label>
            <div className="space-y-3">
              <Button type="button" variant="outline" className="w-full" disabled={locating} onClick={() => geoLocate("geo_ort")}>
                {locating ? <IconLoader2 className="h-4 w-4 mr-1.5 animate-spin" /> : <IconCrosshair className="h-4 w-4 mr-1.5" />}
                Aktuellen Standort verwenden
              </Button>
              {geoFromPhoto && fields.geo_ort && (
              ...
              {fields.geo_ort?.lat != null && fields.geo_ort?.long != null && (
                <GeoMapPicker
                  lat={fields.geo_ort.lat}
                  lng={fields.geo_ort.long}
                  onChange={(lat, lng) => handleMapMove("geo_ort", lat, lng)}
                />
              )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.geo_ort?.lat ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, geo_ort: { ...(f.geo_ort as any ?? {}), lat: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.geo_ort?.long ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, geo_ort: { ...(f.geo_ort as any ?? {}), long: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="kapazitaet">Kapazität (max. Personen)</Label>
            <Input
              id="kapazitaet"
              type="number"
          ...
          <div className="space-y-2">
            <Label htmlFor="notizen_ort">Notizen zum Veranstaltungsort</Label>
            <Textarea
              id="notizen_ort"
              value={fields.notizen_ort ?? ''}
              onChange={e => setFields(f => ({ ...f, notizen_ort: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
